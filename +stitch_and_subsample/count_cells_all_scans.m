function count_cells_all_scans(varargin)
    ...
    for i = 1:numel(paths)
        ...
        for j = 1:numel(list_stitch_info)
            ...
            % load the stitches:
            stitch = stitch_and_subsample.utilities.load_structure_from_file(fullfile(path_scan, name_stitch));
            ...
            [stitch_cropped, cells_algorithm1, cells_algorithm2] = stitch_and_subsample.count_cells_all_scans.get_example(stitch);
            ...
            switch answer
                ...
                case algorithm1 
                    ...
                    gaussian_sigma = stitch_and_subsample.count_cells_all_scans.set_gaussian_size(stitch_small, stitch, stitch_info.scale_rows, stitch_info.scale_columns);

                    %%% Next, count the cells in each individual position.

                    % display status:
                    stitch_and_subsample.utilities.display_status('Counting cells in', name_stitch, path_scan);

                    % count the cells:
                    cells = struct;
                    cells.all.stitch = stitch_and_subsample.count_cells_all_scans.count_cells_in_scan(stitch, gaussian_sigma);
                    ...
                    % overlay on stitch:
                    stitch_small_annotated = stitch_and_subsample.count_cells_all_scans.overlay_on_image(stitch_small, cells.all.stitch_small);
                ...
                case algorithm2
                    ...
                    [sensitivity, minNucleusSize] = stitch_and_subsample.count_cells_all_scans.set_adaptThresh(stitch_small, stitch, stitch_info.scale_rows, stitch_info.scale_columns);
                    ...
                    % count the cells:
                    cells = struct;
                    cells.all.stitch = stitch_and_subsample.count_cells_all_scans.count_cells_in_scan_adaptThresh(stitch, sensitivity, minNucleusSize);
