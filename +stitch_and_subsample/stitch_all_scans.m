function stitch_all_scans(varargin)
    ...
    stitch_info = stitch_and_subsample.stitch_all_scans.create_structure_to_store_stitch_info(paths);

    %%% Next, we need the user to input what channel each wavelength corresponds to.
    
    stitch_info = stitch_and_subsample.stitch_all_scans.get_wavelength_channels(stitch_info);
    ...
    stitch_info = stitch_and_subsample.stitch_all_scans.get_scan_dimensions(stitch_info);
    
    %%% Next, we need get the amount to shift the images by. 
    
    stitch_info = stitch_and_subsample.stitch_all_scans.get_shifts_to_align(stitch_info);
    
    %%% Next, we need to set up the coordinates needed to stitch the
    %%% images.
    
    stitch_info = stitch_and_subsample.stitch_all_scans.get_stitch_coordinates(stitch_info);
    
    %%% Next, we need to create the stitched images. 
    
    stitch_and_subsample.stitch_all_scans.perform_stitching(stitch_info);
