 function segment_all_scans(varargin)
    ...
    paths = stitch_and_subsample.utilities.get_paths_to_data(varargin);
    ...
    for i = 1:numel(paths)
        ...
        for j = 1:numel(list_stitch_info)
            ...
            stitch_info = stitch_and_subsample.utilities.load_structure_from_file(fullfile(path_scan, list_stitch_info(j).name));
            
            % get the name of the scan:
            name_scan = stitch_info.name_scan;
            
            % load the cells:
            cells = stitch_and_subsample.utilities.load_structure_from_file(fullfile(path_scan, sprintf('Cell_Info_%s.mat', name_scan)));
            ...
            if exist(fullfile(path_scan, file_name_boundaries), 'file') == 2
                ...
                % load the boundaries:
                boundaries = stitch_and_subsample.utilities.load_structure_from_file(fullfile(path_scan, file_name_boundaries));
            ...
            boundaries.well.stitch_small = stitch_and_subsample.segment_all_scans.gui_to_segment_a_stitch(stitch_small, boundaries.well.stitch_small, instructions_well);
            ...
            if strcmp(answer, 'Yes')
                ...
                boundaries.colonies.stitch_small = stitch_and_subsample.segment_all_scans.guess_colonies(cells.all.stitch_small, boundaries.colonies.stitch_original, stitch_small, boundaries.well.stitch_small);
            
            end
            
            % segment the colonies:
            instructions_colonies = 'colonies';
            boundaries.colonies.stitch_small = stitch_and_subsample.segment_all_scans.gui_to_segment_a_stitch(stitch_small, boundaries.colonies.stitch_small, instructions_colonies);
            ...
            boundaries = stitch_and_subsample.segment_all_scans.scale_coords_boundary_up(boundaries, stitch_info.scale_rows, stitch_info.scale_columns);
            
            % determine cells in the well/colonies:
            cells.well = stitch_and_subsample.segment_all_scans.get_cells_within_boundaries(cells.all, boundaries.well);
            cells.colonies = stitch_and_subsample.segment_all_scans.get_cells_within_boundaries(cells.all, boundaries.colonies);
            ...
            % overlay on stitch:
            stitch_small_annotated = stitch_and_subsample.segment_all_scans.overlay_on_image(stitch_small, cells);
