function stitch_info = get_wavelength_channels(stitch_info)
    ...
    if strcmp(answer, 'Yes, all do.')
        ...
        % get channel names:
        channels = stitch_and_subsample.stitch_all_scans.get_wavelength_channels.get_channels_for_each_wavelength('ALL FOLDERS', 'ALL SCANS', stitch_info(1).scan_info(1).images);
    ...
    for i = 1:numel(stitch_info)
        ...
        for j = 1:numel(stitch_info(i).scan_info)
            ...
            if strcmp(answer, 'No, none do.')
                ...
                % get channel names:
                channels = stitch_and_subsample.stitch_all_scans.get_wavelength_channels.get_channels_for_each_wavelength(stitch_info(i).path_folder, stitch_info(i).scan_info(j).name_scan, stitch_info(i).scan_info(j).images);
