function stitch_a_wavelength(scan, images)
    ...
    for j = 1:num_positions
        ...
        temp_image_name = sprintf('%s%s%s', images.wavelength, num2str(sprintf('%04d',j)), scan.name_scan);
    ...
    % set file name:
    file_name = fullfile(scan.path_folder, sprintf('Stitch_Original_%s.mat', images.channel));
    file_name_full = fullfile(scan.path_folder, sprintf('Stitch_Original_%s.tif', images.channel));
    file_name_small = fullfile(scan.path_folder, sprintf('Stitch_Small_%s.tif', images.channel));
    ...
    % save larger image as tif:
    imwrite(image_to_save, file_name_full);
