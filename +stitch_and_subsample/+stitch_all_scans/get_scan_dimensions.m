function stitch_info = get_scan_dimensions(stitch_info)
    ...
    if strcmp(answer, 'Yes, all do.')
        ...
        % get scan size:
        [num_rows, num_columns] = stitch_and_subsample.stitch_all_scans.get_scan_dimensions.enter_scan_dimensions('ALL FOLDERS', 'ALL SCANS');
    ...
    for i = 1:numel(stitch_info)
        ...
        for j = 1:numel(stitch_info(i).scan_info)
            ...
            if strcmp(answer, 'No, none do.')
                ...
                % get channel names:
                [num_rows, num_columns] = stitch_and_subsample.stitch_all_scans.get_scan_dimensions.enter_scan_dimensions(stitch_info(i).path_folder, stitch_info(i).scan_info(j).name_scan);
            ...
            for k = 1:numel(stitch_info(i).scan_info(j).images)
                ...
                [~, rows_to_remove] = stitch_and_subsample.utilities.get_structure_results_containing_string(list_images, 'name', num2str((num_rows*num_columns) + 1));
