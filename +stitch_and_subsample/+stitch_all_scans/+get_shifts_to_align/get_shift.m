function [shift_column, shift_row, image_height, image_width] = get_shift(images, num_rows, num_columns, path_images, name_folder, name_scan)
    ...
    % otherwise:
        ...
        % use dapi:
        list_images = stitch_and_subsample.utilities.get_structure_results_matching_string(images, 'channel', 'dapi');
    ...
    matrix_of_positions = reshape(1:num_rows*num_columns, [num_rows,num_columns])';
    matrix_of_positions(2:2:end,:) = fliplr(matrix_of_positions(2:2:end,:));
    ...
    % get the images:
    [image_middle, image_below, image_right] = stitch_and_subsample.stitch_all_scans.get_shifts_to_align.get_images_from_tile_numbers(tile_row_middle, tile_column_middle, matrix_of_positions, list_images, path_images);
    ...
    switch answer
        ...
        case option_alignment_manual 
            ...
            shift_column = stitch_and_subsample.stitch_all_scans.get_shifts_to_align.get_shift_visually(image_middle, image_below);
            
            % get row shift distances:
            shift_row = stitch_and_subsample.stitch_all_scans.get_shifts_to_align.get_shift_visually(image_middle, image_right);
        ...
        case option_alignment_overlap
            ...
            % get coordinates:
            [shift_column, shift_row] = stitch_and_subsample.stitch_all_scans.get_shifts_to_align.get_shift_overlap(name_folder, name_scan);
