function boundaries = guess_colonies(cells, boundaries, stitch_small, boundary_well)
    ...
    % get density image:
    image_density = stitch_and_subsample.segment_all_scans.guess_colonies.create_density_image(image, 5);
    ...
    [colony_centroids_x, colony_centroids_y] = stitch_and_subsample.segment_all_scans.guess_colonies.get_colony_centroids(image_threshold);
    
    % get number of colonies:
    num_colonies = numel(colony_centroids_x);
    
    % assign cells to colonies:
    colony_assignment = stitch_and_subsample.segment_all_scans.guess_colonies.assign_cells_to_centroids(cells_x_rounded, cells_y_rounded, image_threshold, colony_centroids_x, colony_centroids_y);
    
    % use the cell assignments to get colony boundaries:
    boundaries = stitch_and_subsample.segment_all_scans.guess_colonies.get_colony_boundaries(boundaries, cells_x_rounded, cells_y_rounded, colony_assignment, stitch_small);
