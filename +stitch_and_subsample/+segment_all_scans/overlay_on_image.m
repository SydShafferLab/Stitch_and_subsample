function image_annotated = overlay_on_image(image, cells)
    ...
    for i = 1:num_colonies
        ...
        if size(cells.colonies(i).boundary_stitch_small, 1) > 5
            % add boundary to image:
            image_annotated = stitch_and_subsample.utilities.add_boundary_to_image(image_annotated, cells.colonies(i).boundary_stitch_small, color.colonies(i, :), marker_size);
        end   
    end
    ...
    for i = 1:num_colonies
        ...
        if size(cells.colonies(i).boundary_stitch_small, 1) > 5
            % add cells to image:
            image_annotated = stitch_and_subsample.utilities.add_cells_to_image(image_annotated, cells.colonies(i).cells_stitch_small, color.colonies(i, :), marker_size);
        end
    end
